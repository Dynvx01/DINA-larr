...
if (passwordInput === correctPassword) {
    document.getElementById("passwordScreen").style.display = "none"; // Sembunyikan form password
    document.getElementById("opening").style.display = "block"; // Tampilkan pesan selamat datang
    const audio = document.getElementById("backgroundMusic");
    audio.play(); // Putar audio
    setTimeout(() => {
        document.getElementById("opening").style.display = "none"; // Sembunyikan pesan selamat datang setelah 2 detik
        document.querySelector(".container").style.display = "block"; // Tampilkan konten utama
    }, 2000); // Ubah waktu (dalam ms) sesuai keinginan
} 
...
const messages = [
    ...
    "Makasih atas Waktunya Maaf yaaa, ohh iyaa soal-soalnya jangan lupa dipelajarin, soalnya yaa kelebihanku itu membantu ^o^. Eh btw gimana gambarannya gak mirip yaa? Sorry yaa wkwk. Sekali lagi tolong jangan asing yaa walaupun kita bukan siapa-siapa hehee. ^_^",
];
